...
export default function Information(props) {
    ...
    return (
        <main className='flex-1  p-4 flex flex-col gap-3 text-center sm:gap-4 justify-center pb-20 max-w-prose w-full mx-auto'>
            ...
            <div className='grid grid-cols-2 sm:mx-auto bg-white rounded-full overflow-hidden items-center blueShadow '>
                <button onClick={() => setTab('transcription')} 
                className={'px-4 rounded duration-200 py-1 ' + (tab === 'transcription' ? ' bg-blue-400 text-white' : 'hover:text-blue-600')}>
                    Transcription
                </button>
                <button onClick={() => setTab('translation')} 
                className={'px-4 rounded duration-200 py-1  ' + (tab === 'translation' ? ' bg-blue-400 text-white' : 'hover:text-blue-600')}>
                    Translation
                </button>
            </div>
